;;;; src/crypto/bcrypt.lisp — Bcrypt password hashing (Provos/Mazieres)
;;;; Full Blowfish implementation from scratch with EksBlowfishSetup.
(in-package :cauldron.crypto)

;;; ---------- Blowfish Initial P-array (18 entries) ----------
;;; Derived from hexadecimal digits of pi.
...
;;; ---------- Blowfish Initial S-boxes (4 x 256 entries each) ----------
;;; Also derived from digits of pi.
...
;;; Different from standard base64! Order: ./A-Za-z0-9

(defvar +bcrypt-base64-alphabet+
  "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")

(defvar *bcrypt-base64-decode-table*
  (let ((table (make-array 256 :element-type '(signed-byte 8) :initial-element -1)))
    (loop for i from 0 below 64
          do (setf (aref table (char-code (char +bcrypt-base64-alphabet+ i))) i))
    table))

;;; ---------- bcrypt Base64 Encoding/Decoding ----------

(defun bcrypt-base64-encode (octets num-chars)
  "Encode OCTETS using bcrypt's non-standard base64, emitting NUM-CHARS characters."
  (let ((result (make-string num-chars))
    ...
    (loop for i from 0 below len by 3
          ...
          do ;; Encode in bcrypt order (LSB first per byte)
             (when (< pos num-chars)
               (setf (char result pos) (char +bcrypt-base64-alphabet+ (logand b0 #x3F)))
               (incf pos))
             (when (< pos num-chars)
               (setf (char result pos)
                     (char +bcrypt-base64-alphabet+
                           (logior (ash (logand b0 #xC0) -6)
                                   (ash (logand b1 #x0F) 2))))
               (incf pos))
             (when (< pos num-chars)
               (setf (char result pos)
                     (char +bcrypt-base64-alphabet+
                           (logior (ash (logand b1 #xF0) -4)
                                   (ash (logand b2 #x03) 4))))
               (incf pos))
             (when (< pos num-chars)
               (setf (char result pos)
                     (char +bcrypt-base64-alphabet+ (ash (logand b2 #xFC) -2)))
               (incf pos)))
    result))

(defun bcrypt-base64-decode (string num-bytes)
  "Decode STRING using bcrypt's non-standard base64, returning NUM-BYTES octets."
  (let ((result (make-array num-bytes :element-type '(unsigned-byte 8) :initial-element 0))
        (pos 0)
        (len (length string)))
    (loop for i from 0 below len by 4
          while (< pos num-bytes)
          for c0 = (aref *bcrypt-base64-decode-table* (char-code (char string i)))
          for c1 = (if (< (+ i 1) len)
                       (aref *bcrypt-base64-decode-table* (char-code (char string (+ i 1)))) 0)
          for c2 = (if (< (+ i 2) len)
                       (aref *bcrypt-base64-decode-table* (char-code (char string (+ i 2)))) 0)
          for c3 = (if (< (+ i 3) len)
                       (aref *bcrypt-base64-decode-table* (char-code (char string (+ i 3)))) 0)
          do (when (< pos num-bytes)
               (setf (aref result pos)
                     (logand #xFF (logior c0 (ash (logand c1 #x03) 6))))
               (incf pos))
             (when (< pos num-bytes)
               (setf (aref result pos)
                     (logand #xFF (logior (ash (logand c1 #x3C) -2)
                                         (ash (logand c2 #x0F) 4))))
               (incf pos))
             (when (< pos num-bytes)
               (setf (aref result pos)
                     (logand #xFF (logior (ash (logand c2 #x30) -4)
                                         (ash c3 2))))
               (incf pos)))
    result))
...
(defun bf-make-state ()
  "Create a fresh Blowfish state with P-array and S-boxes initialized from pi digits."
  (let ((p (make-array 18 :element-type '(unsigned-byte 32)))
        (s0 (make-array 256 :element-type '(unsigned-byte 32)))
        (s1 (make-array 256 :element-type '(unsigned-byte 32)))
        (s2 (make-array 256 :element-type '(unsigned-byte 32)))
        (s3 (make-array 256 :element-type '(unsigned-byte 32))))
    (replace p +bf-p-init+)
    (replace s0 +bf-s0-init+)
    (replace s1 +bf-s1-init+)
    (replace s2 +bf-s2-init+)
    (replace s3 +bf-s3-init+)
    (list p s0 s1 s2 s3)))

(defun bf-f (s0 s1 s2 s3 x)
  "Blowfish F function: split X into 4 bytes a,b,c,d and compute
((S0[a] + S1[b]) XOR S2[c]) + S3[d] mod 2^32."
  (let ((a (logand (ash x -24) #xFF))
        (b (logand (ash x -16) #xFF))
        (c (logand (ash x -8)  #xFF))
        (d (logand x #xFF)))
    (logand #xFFFFFFFF
            (+ (logxor (logand #xFFFFFFFF
                               (+ (aref s0 a) (aref s1 b)))
                       (aref s2 c))
               (aref s3 d)))))

(defun bf-encrypt-pair (state l r)
  "Blowfish-encrypt a single 64-bit block (L, R). Returns (values new-L new-R).
STATE is (list P S0 S1 S2 S3)."
  (let ((p  (first state))
        (s0 (second state))
        (s1 (third state))
        (s2 (fourth state))
    ...
    (loop for i from 0 below 16
          do (setf l (logand #xFFFFFFFF (logxor l (aref p i))))
             (setf r (logand #xFFFFFFFF (logxor r (bf-f s0 s1 s2 s3 l))))
             ;; Swap L and R
             (rotatef l r))
    ;; Undo last swap
    (rotatef l r)
    ;; Final XOR
    (setf r (logand #xFFFFFFFF (logxor r (aref p 16))))
    (setf l (logand #xFFFFFFFF (logxor l (aref p 17))))
    (values l r)))

;;; ---------- Key Schedule Helpers ----------

(defun bf-expand-key (state key)
  "Standard Blowfish key expansion: XOR key bytes into P-array, then
encrypt-and-replace all P and S-box entries."
  (let ((p  (first state))
        (s0 (second state))
        (s1 (third state))
        (s2 (fourth state))
        (s3 (fifth state))
        (key-len (length key)))
    ;; XOR key into P-array (cycling through key bytes, 4 bytes per P entry)
    (let ((key-pos 0))
      (loop for i from 0 below 18
            for word = (let ((w 0))
                         (loop for j from 0 below 4
                               do (setf w (logand #xFFFFFFFF
                                                  (logior (ash w 8)
                                                          (aref key (mod key-pos key-len)))))
                                  (incf key-pos))
                         w)
            do (setf (aref p i) (logand #xFFFFFFFF (logxor (aref p i) word)))))
    ;; Encrypt-and-replace P-array entries
    (let ((l 0) (r 0))
      (loop for i from 0 below 18 by 2
            do (multiple-value-setq (l r) (bf-encrypt-pair state l r))
               (setf (aref p i) l)
               (setf (aref p (+ i 1)) r))
      ;; Encrypt-and-replace S-box entries
      (dolist (s (list s0 s1 s2 s3))
        (loop for i from 0 below 256 by 2
              do (multiple-value-setq (l r) (bf-encrypt-pair state l r))
                 (setf (aref s i) l)
                 (setf (aref s (+ i 1)) r))))))

(defun bf-expand-key-with-salt (state key salt)
  "Expand key XORing salt into the encrypt-and-replace phase (EksBlowfish).
KEY and SALT are octet vectors."
  (let ((p  (first state))
        (s0 (second state))
        (s1 (third state))
        (s2 (fourth state))
    ...
    (let ((key-pos 0))
      (loop for i from 0 below 18
            for word = (let ((w 0))
                         ...
                         w)
            do (setf (aref p i) (logand #xFFFFFFFF (logxor (aref p i) word)))))
    ;; Encrypt-and-replace P-array, XORing salt into L,R each round
    (let ((l 0) (r 0) (salt-pos 0))
      (loop for i from 0 below 18 by 2
            do ;; XOR 4 bytes of salt into L
               (let ((sw 0))
                 (loop for j from 0 below 4
                       do (setf sw (logand #xFFFFFFFF
                                           (logior (ash sw 8)
                                                   (aref salt (mod salt-pos salt-len)))))
                          (incf salt-pos))
                 (setf l (logand #xFFFFFFFF (logxor l sw))))
               ;; XOR 4 bytes of salt into R
               (let ((sw 0))
                 (loop for j from 0 below 4
                       do (setf sw (logand #xFFFFFFFF
                                           (logior (ash sw 8)
                                                   (aref salt (mod salt-pos salt-len)))))
                          (incf salt-pos))
                 (setf r (logand #xFFFFFFFF (logxor r sw))))
               (multiple-value-setq (l r) (bf-encrypt-pair state l r))
               (setf (aref p i) l)
               (setf (aref p (+ i 1)) r))
      ;; Encrypt-and-replace S-box entries, same salt-XOR pattern
      (dolist (s (list s0 s1 s2 s3))
        (loop for i from 0 below 256 by 2
              do (let ((sw 0))
                   (loop for j from 0 below 4
...
(defun eks-blowfish-setup (cost salt key)
  "Expensive key schedule for bcrypt.
COST is the log2 iteration count. SALT is 16-byte octet vector.
KEY is password bytes (with trailing NUL)."
  (let ((state (bf-make-state))
        (rounds (ash 1 cost)))
    ;; Initial key expansion with salt
    (bf-expand-key-with-salt state key salt)
    ;; Expensive iteration: alternate re-keying with password and salt
    (loop for i from 0 below rounds
          do (bf-expand-key state key)
             (bf-expand-key state salt))
    state))

;;; ---------- bcrypt Core ----------

;; "OrpheanBeholderScryDoubt" as bytes
(defvar +bcrypt-ciphertext+
  #(#x4F #x72 #x70 #x68 #x65 #x61 #x6E #x42
    #x65 #x68 #x6F #x6C #x64 #x65 #x72 #x53
    #x63 #x72 #x79 #x44 #x6F #x75 #x62 #x74))

(defun bcrypt-encrypt-ctext (state)
  "Encrypt the bcrypt plaintext 'OrpheanBeholderScryDoubt' 64 times.
Returns 24 bytes as an octet vector."
  (let ((ctext (make-array 6 :element-type '(unsigned-byte 32))))
    ;; Load 24 bytes as 6 big-endian 32-bit words
    (loop for i from 0 below 6
          for offset = (* i 4)
          do (setf (aref ctext i)
                   (logior (ash (aref +bcrypt-ciphertext+ offset) 24)
                           (ash (aref +bcrypt-ciphertext+ (+ offset 1)) 16)
                           (ash (aref +bcrypt-ciphertext+ (+ offset 2)) 8)
                           (aref +bcrypt-ciphertext+ (+ offset 3)))))
    ;; Encrypt 64 times (3 pairs of 32-bit words)
    (loop for round from 0 below 64
          do (loop for i from 0 below 6 by 2
                   do (multiple-value-bind (l r)
                          (bf-encrypt-pair state (aref ctext i) (aref ctext (+ i 1)))
                        (setf (aref ctext i) l)
                        (setf (aref ctext (+ i 1)) r))))
    ;; Convert back to 24 bytes (big-endian)
    (let ((result (make-array 24 :element-type '(unsigned-byte 8))))
      (loop for i from 0 below 6
            for val = (aref ctext i)
            for offset = (* i 4)
            do (setf (aref result offset)       (logand (ash val -24) #xFF))
               (setf (aref result (+ offset 1)) (logand (ash val -16) #xFF))
               (setf (aref result (+ offset 2)) (logand (ash val -8)  #xFF))
               (setf (aref result (+ offset 3)) (logand val #xFF)))
      result)))

;;; ---------- Public API ----------

(defun bcrypt-hash (password &key (cost 12))
  "Hash PASSWORD (string) using bcrypt with given COST factor (4-31, default 12).
Returns a bcrypt hash string in the format $2b$COST$SALT+HASH."
  (assert (<= 4 cost 31) (cost) "bcrypt cost must be between 4 and 31, got ~D" cost)
  (let* ((salt-bytes (secure-random-bytes 16))
         ;; Password bytes: UTF-8 encoded with trailing NUL, max 72 bytes
         (pw-octets (ensure-octets password))
         (pw-with-nul (let* ((pw-len (min (length pw-octets) 72))
                             (key (make-array (+ pw-len 1) :element-type '(unsigned-byte 8)
                                              :initial-element 0)))
                        (replace key pw-octets :end2 pw-len)
                        key))
         ;; Run EksBlowfishSetup
         (state (eks-blowfish-setup cost salt-bytes pw-with-nul))
         ;; Encrypt ciphertext
         (hash-bytes (bcrypt-encrypt-ctext state))
         ;; Format output
         (cost-str (format nil "~2,'0D" cost))
         (salt-str (bcrypt-base64-encode salt-bytes 22))
         ;; bcrypt uses 23 chars of hash (encoding first 23 bytes of the 24-byte output)
         (hash-str (bcrypt-base64-encode hash-bytes 31)))
    (concatenate 'string "$2b$" cost-str "$" salt-str hash-str)))

(defun bcrypt-verify (password hash-string)
  "Verify PASSWORD (string) against a bcrypt HASH-STRING. Returns T or NIL."
  (when (and (stringp hash-string)
             (>= (length hash-string) 59)  ;; $2b$XX$ = 7 chars + 22 salt + 31 hash = 60 min
             (char= (char hash-string 0) #\$))
    (let* (;; Parse: $2b$COST$SALTHASH or $2a$COST$SALTHASH
           (cost-str (subseq hash-string 4 6))
           (cost (parse-integer cost-str))
           (salt-str (subseq hash-string 7 29))     ;; 22 chars
           (hash-str (subseq hash-string 29))       ;; 31 chars
           ;; Decode salt
           (salt-bytes (bcrypt-base64-decode salt-str 16))
           ...
           (pw-with-nul (let* ((pw-len (min (length pw-octets) 72))
                          ...
                          (replace key pw-octets :end2 pw-len)
                          key))
           ;; Run EksBlowfishSetup with parsed cost and salt
           (state (eks-blowfish-setup cost salt-bytes pw-with-nul))
           ;; Encrypt ciphertext
           (computed-bytes (bcrypt-encrypt-ctext state))
           (computed-str (bcrypt-base64-encode computed-bytes 31)))
      ;; Constant-time comparison
      (secure-equal computed-str hash-str))))
